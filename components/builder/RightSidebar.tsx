...
import { MousePointerClick, Upload, ShieldCheck, Pencil, CheckCircle2, AlertCircle, AlertTriangle, Minus, LayoutTemplate, ChevronDown, X, Plus, Sparkles, Loader2, Link2, Link2Off, Palette, Settings2 } from 'lucide-react';
...
function BrandSwatches({ onSelect }: { onSelect: (token: string) => void }) {
  ...
  const swatches: Array<{ token: string; label: string; color: string }> = [
    { token: '$primary', label: 'Primary', color: brandKit.primaryColor },
    { token: '$secondary', label: 'Secondary', color: brandKit.secondaryColor },
    { token: '$background', label: 'Background', color: brandKit.backgroundColor },
    { token: '$text', label: 'Text', color: brandKit.textColor },
    ...(brandKit.customColors ?? []).map((c, i) => ({ token: `$custom${i + 1}`, label: `Custom ${i + 1}`, color: c })),
  ];
  ...
  return (
    <div ref={ref} className="relative flex-shrink-0">
      <button
        ...
        className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${open ? 'bg-[#6366f1]/20 text-[#818cf8]' : 'bg-[#222226] text-[#52525b] hover:text-[#a1a1aa]'}`}
      >
        <Palette className="w-3 h-3" />
      </button>
      {open && (
        <div className="absolute bottom-full right-0 mb-1 z-50 bg-[#18181b] border border-[#2a2a2e] rounded-lg shadow-xl p-2 min-w-[140px]">
          <p className="text-[9px] font-semibold text-[#52525b] uppercase tracking-wider px-1 pb-1.5">Brand colors</p>
          <div className="space-y-0.5">
            {swatches.map(({ token, label, color }) => (
              <button
                key={token}
                onClick={() => { onSelect(token); setOpen(false); }}
                className="w-full flex items-center gap-2 px-1.5 py-1 rounded hover:bg-[#2a2a2e] transition-colors"
              >
                <div className="w-3.5 h-3.5 rounded-sm border border-[#3a3a3e] flex-shrink-0" style={{ backgroundColor: color }} />
                <span className="text-[10px] text-[#a1a1aa]">{label}</span>
                <span className="text-[9px] text-[#52525b] ml-auto font-mono">{token}</span>
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <div data-tour="inspector" className="w-64 flex-shrink-0 bg-[#161618] flex flex-col overflow-hidden">
      ...
      <div className="px-2 py-2 border-b border-[#2a2a2e] flex-shrink-0">
        <div role="tablist" aria-label="Inspector panels" className="flex items-center gap-0.5 bg-[#0f0f11] rounded-lg p-0.5">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 flex-1 justify-center py-1 rounded-md text-xs font-medium transition-all duration-100 ${
              tab === 'advanced' ? 'bg-[#222226] text-[#f4f4f5] shadow-sm' : 'text-[#71717a] hover:text-[#a1a1aa]'
            }`}
          >
      ...
      <div className="flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto h-full">
            {!selectedRow ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 px-4">
                ...
                <div className="text-center">
                  <p className="text-xs font-medium text-[#71717a]">No section selected</p>
                  <p className="text-[10px] text-[#3a3a3e] mt-1 leading-relaxed max-w-[140px]">Select a section to adjust spacing</p>
                </div>
              </div>
            ) : (
              <>
                {/* Row / Section spacing */}
                <CollapsibleGroup title="Row / Section" defaultOpen={true}>
                  <p className="text-[9px] font-semibold text-[#52525b] uppercase tracking-wider mb-2">Padding</p>
                  <FourSidedPadding
                    linked={rowPadLinked}
                  ...
                  <div className="mt-3 pt-3 border-t border-[#2a2a2e]">
                    <p className="text-[9px] font-semibold text-[#52525b] uppercase tracking-wider mb-2">Margin</p>
                    <FourSidedPadding
                      linked={false}
                      onToggleLink={() => {}}
                      top={selectedRow.outerMarginTop ?? 0}
                      right={selectedRow.outerMarginRight ?? 0}
                      bottom={selectedRow.outerMarginBottom ?? 0}
                      left={selectedRow.outerMarginLeft ?? 0}
                      onChange={(side, v) => {
                        const key = side === 'top' ? 'outerMarginTop' : side === 'right' ? 'outerMarginRight' : side === 'bottom' ? 'outerMarginBottom' : 'outerMarginLeft';
                        selected && updateRowSpacing(selected.rowId, { [key]: v });
                      }}
                    />
                  </div>
                ...
                {/* Element spacing */}
                {selectedColumn && selectedColumn.type !== 'empty' && selectedColumn.type !== 'spacer' && (
                  <CollapsibleGroup title="Element" defaultOpen={true}>
                    <p className="text-[9px] font-semibold text-[#52525b] uppercase tracking-wider mb-2">Padding</p>
                    <FourSidedPadding
                    ...
                    <div className="mt-3 pt-3 border-t border-[#2a2a2e]">
                      ...
                      <FourSidedPadding
                        linked={false}
                        onToggleLink={() => {}}
                        top={selectedColumn.content.sectionMarginTop ?? 0}
                        right={selectedColumn.content.sectionMarginRight ?? 0}
                        bottom={selectedColumn.content.sectionMarginBottom ?? 0}
                        left={selectedColumn.content.sectionMarginLeft ?? 0}
                        onChange={(side, v) => {
                          const key = side === 'top' ? 'sectionMarginTop' : side === 'right' ? 'sectionMarginRight' : side === 'bottom' ? 'sectionMarginBottom' : 'sectionMarginLeft';
                          selected && updateColumnContent(selected.rowId, selected.colIdx, { [key]: v });
                        }}
                      />
                    </div>
                    {selectedRow.columns.length > 1 && (
                      <div className="mt-3 pt-3 border-t border-[#2a2a2e]">
                        <p className="text-[9px] font-semibold text-[#52525b] uppercase tracking-wider mb-2">Column cell padding</p>
                        <FourSidedPadding
                          linked={colPadLinked}
        ...
        {/* ── A11y tab (includes compatibility) ── */}
        {tab === 'a11y' && (
